...
# Tabs
# ---------------------------
tab1, tab2, tab3, tab4 = st.tabs(["📊 Overview", "🎯 Long Straddle", "🔀 Long Strangle", "📁 Export"])
...
with tab1:
    ...
    with col1:
        st.markdown("### 🎯 Long Straddle Strategy")
        st.markdown(f"""
        ...
        **Entry:**
        ...
        - 💰 Entry Cost = 2 × ATM Premium × {contracts} × {lot_size}
        
        **Exit:**
        - 💵 Sell both options at end of day
        ...
        **Example:**
        - Spot: 24,000
        - ATM Strike: 24,000
        - Call Premium: ₹150, Put Premium: ₹150
        - Entry Cost: ₹{150*2*contracts*lot_size:,}
        """)
    
    with col2:
        st.markdown("### 🔀 Long Strangle Strategy")
        st.markdown(f"""
        **Concept:** Buy OTM Call + OTM Put
        
        **Entry:**
        - 📍 Call Strike: {otm_percent}% above spot
        - 📍 Put Strike: {otm_percent}% below spot
        - 📊 Buy both OTM options
        - 💰 Entry Cost = (OTM Call + OTM Put) × {contracts} × {lot_size}
        ...
        **Example:**
        ...
        - Call Premium: ₹80, Put Premium: ₹80
        - Entry Cost: ₹{(80+80)*contracts*lot_size:,}
        """)
...
# Long Straddle
with tab2:
    st.subheader("🎯 Long Straddle Backtest")
    ...
    **Strategy:** Buy ATM Call + ATM Put on GANN dates
    ...
    if st.button("▶️ Run Straddle Backtest", key="straddle_btn"):
...
# Long Strangle
with tab3:
    st.subheader("🔀 Long Strangle Backtest")
    ...
    **Strategy:** Buy OTM Call ({otm_percent}% above) + OTM Put ({otm_percent}% below) on GANN dates
    ...
    if st.button("▶️ Run Strangle Backtest", key="strangle_btn"):
...
with tab4:
    ...
    with col1:
        if 'straddle_results' in st.session_state:
            ...
            download_link = f'<a href="data:file/csv;base64,{b64}" download="straddle_results_{datetime.now().strftime("%Y%m%d")}.csv" style="text-decoration:none;"><button style="background:#7dd3fc;color:#012;padding:10px 20px;border:none;border-radius:5px;font-weight:700;">📥 Download Straddle CSV</button></a>'
    ...
    with col2:
        if 'strangle_results' in st.session_state:
            ...
            b64 = base64.b64encode(csv.encode()).decode()
            download_link = f'<a href="data:file/csv;base64,{b64}" download="strangle_results_{datetime.now().strftime("%Y%m%d")}.csv" style="text-decoration:none;"><button style="background:#a78bfa;color:#012;padding:10px 20px;border:none;border-radius:5px;font-weight:700;">📥 Download Strangle CSV</button></a>'
            st.markdown(download_link, unsafe_allow_html=True)
        else:
...
<div style='text-align: center; color: var(--muted); font-size: 12px; padding: 20px 0;'>
    ...
    <p>⚠️ Disclaimer: Backtesting uses estimated option prices. Real trading results may vary. Not financial advice.</p>
